...
public class DefaultAuthenticationCeremonyServicePackedEd25519Tests : AbstractAuthenticationCeremonyServiceTests
{
    ...
    {
        await SetupEd25519RegistrationAsync();
    }

    private async Task SetupEd25519RegistrationAsync()
    {
    ...
    [Test]
    public async Task DefaultAuthenticationCeremonyService_PerformsCeremonyWithoutErrorsForPacked_WhenEc25519()
    {
        ...
        AuthenticationCeremonyStorage.ReplaceChallengeForAuthenticationCeremonyOptions(
            ...
            WebEncoders.Base64UrlDecode("1lLmePx6k85VcLZhlf1jQg"));

        var completeResult = await AuthenticationCeremonyService.CompleteCeremonyAsync(
            new DefaultHttpContext(new FeatureCollection()),
            new(beginResult.AuthenticationCeremonyId,
                new("cNOQx_X1dUKCYDdmFfQ4v6FS0xc_D-IlzHkXgI-EgTXs-ah8e_KCSou1igeou7iq",
                    "cNOQx_X1dUKCYDdmFfQ4v6FS0xc_D-IlzHkXgI-EgTXs-ah8e_KCSou1igeou7iq",
                    new("eyJ0eXBlIjoid2ViYXV0aG4uZ2V0IiwiY2hhbGxlbmdlIjoiMWxMbWVQeDZrODVWY0xaaGxmMWpRZyIsIm9yaWdpbiI6Imh0dHBzOi8vc3VubnktcmlnaHRseS1jaXZldC5uZ3Jvay1mcmVlLmFwcCIsImNyb3NzT3JpZ2luIjpmYWxzZX0",
                        "9LDXMfg-tpDkp8NkozQ4wEZ932qRlKLYEvVDB6WER_gFAAAAAw",
                        "7AsQfo9XtAyODYXWAZF40n5npndGQqDMd0n04AHGopmxu9ffEkynC4M5IqHKYTRimodF7XYdvckReod6Dkl8AQ",
